...
export default function CartDrawer() {
  ...
  return (
    <>
      {isOpen && (
        <div className="fixed inset-0 bg-mahogany/50 z-40 animate-fade-in"
      ...
      <div className={`fixed top-0 right-0 h-full w-full max-w-sm bg-cream z-50 shadow-2xl flex flex-col cart-drawer ${
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-parchment-dark">
          ...
          <button onClick={() => setIsOpen(false)} className="p-2 text-mahogany/50 hover:text-mahogany transition-colors">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center gap-4 py-16">
              ...
              <Link to="/shop" onClick={() => setIsOpen(false)} className="btn-outline text-xs px-6 py-2">
                Browse Candles
              </Link>
            </div>
          ) : (
            items.map(item => (
              <div key={item.cartKey} className="flex gap-4 py-4 border-b border-parchment">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {/* Show variant info if this is a t-shirt variant */}
                  {item.variantColor && (
                    <p className="font-raleway text-xs text-gold mt-0.5">
                      {item.variantColor} / {item.variantSize}
                    </p>
                  )}
                  {!item.variantColor && item.size_oz && (
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <button onClick={() => updateQuantity(item.cartKey, item.quantity - 1)}
                      className="w-6 h-6 border border-parchment-dark text-mahogany/60 hover:border-gold hover:text-gold transition-colors flex items-center justify-center text-sm">−</button>
                    <span className="font-raleway text-sm w-6 text-center">{item.quantity}</span>
                    <button onClick={() => updateQuantity(item.cartKey, item.quantity + 1)}
                      className="w-6 h-6 border border-parchment-dark text-mahogany/60 hover:border-gold hover:text-gold transition-colors flex items-center justify-center text-sm">+</button>
                    <button onClick={() => removeItem(item.cartKey)} className="ml-2 text-mahogany/30 hover:text-red-500 transition-colors">
