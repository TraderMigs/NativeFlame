...
export default function Shop() {
  ...
  // Read filter state from URL — single source of truth
  const activeType       = searchParams.get('type')       || ''
  const activeCollection = searchParams.get('collection') || 'All'
  const sortBy           = searchParams.get('sort')       || 'newest'

  // Resolve display label for page header
  ...
  function setCollection(value) {
    const next = new URLSearchParams(searchParams)
    if (value && value !== 'All') next.set('collection', value)
  ...
  // Handle legacy Home page links (?collection=dark / ?collection=natural)
  useEffect(() => {
    ...
    <div className="min-h-screen pt-24 bg-cream">
      ...
      <div className="relative py-20 px-4 text-center overflow-hidden"
        ...
        <div className="relative z-10">
          ...
          <h1 className="font-cinzel text-4xl md:text-5xl font-bold text-cream mb-4">
            {currentTypeLabel}
          </h1>
          ...
          <div className="flex items-center gap-2 mb-6 flex-wrap">
            ...
            {activeType && (
              <span className="inline-flex items-center gap-1 bg-mahogany/10 text-mahogany font-raleway text-xs px-3 py-1">
                {PRODUCT_TYPES.find(t => t.value === activeType)?.label}
