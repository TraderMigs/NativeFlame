...
const db = supabase
const EMPTY = { label: '', icon: '', sort_order: 0, is_active: true }
...
export default function AdminProductTypes() {
  ...
  async function load() {
    setLoading(true)
    const { data } = await db.from('product_types')
  ...
  function openEdit(t) {
    setForm({ label: t.label, icon: t.icon || '', sort_order: t.sort_order, is_active: t.is_active })
  ...
  async function handleSave(e) {
    ...
    const slug = form.label.trim().toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '')
    const payload = {
    ...
    if (!editId) payload.slug = slug

    if (editId) {
      const { slug: _, ...updatePayload } = payload
      await db.from('product_types').update(updatePayload).eq('id', editId)
    } else {
      await db.from('product_types').insert(payload)
    }
    await load(); setShowForm(false); setSaving(false)
  }

  async function handleDelete(id) {
    await db.from('product_types').delete().eq('id', id)
    setDeleteId(null); await load()
  }

  async function toggleActive(t) {
    await db.from('product_types').update({ is_active: !t.is_active }).eq('id', t.id)
    await load()
  }

  async function moveUp(t, idx) {
    if (idx === 0) return
    const prev = types[idx - 1]
    await Promise.all([
      db.from('product_types').update({ sort_order: prev.sort_order }).eq('id', t.id),
      db.from('product_types').update({ sort_order: t.sort_order }).eq('id', prev.id),
    ])
    await load()
  }

  async function moveDown(t, idx) {
    if (idx === types.length - 1) return
    const next = types[idx + 1]
    await Promise.all([
      db.from('product_types').update({ sort_order: next.sort_order }).eq('id', t.id),
      db.from('product_types').update({ sort_order: t.sort_order }).eq('id', next.id),
    ])
    await load()
  }
    ...
    <div className="min-h-screen bg-parchment">
      <header className="bg-mahogany border-b border-cream/10 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="font-cinzel font-bold text-gold tracking-wider">Shop Tabs</h1>
        </div>
        <button onClick={openNew} className="btn-gold text-xs px-5 py-2">+ New Tab</button>
      </header>

      <div className="max-w-3xl mx-auto px-4 md:px-8 py-10">

        <p className="font-lora text-sm italic text-mahogany/50 mb-8">
          These are the tabs that appear on your Shop page. Add, rename, reorder, or hide any tab.
          Changes appear on the shop instantly.
        </p>
          ...
          <div className="fixed inset-0 bg-mahogany/60 z-50 flex items-start justify-center overflow-y-auto py-8 px-4">
            <div className="bg-cream w-full max-w-md shadow-2xl">
              <div className="flex items-center justify-between px-6 py-5 bg-mahogany border-b border-cream/10">
                <h2 className="font-cinzel font-bold text-gold">{editId ? 'Edit Tab' : 'New Tab'}</h2>
                <button onClick={() => setShowForm(false)} className="text-cream/50 hover:text-cream">✕</button>
              </div>
              <form onSubmit={handleSave} className="p-6 space-y-4">

                {/* Live preview */}
                <div className="bg-mahogany px-5 py-3 flex items-center gap-2">
                ...
                <div className="grid grid-cols-4 gap-3">
                  <div className="col-span-3">
                    <label className="font-raleway text-xs uppercase tracking-wider text-mahogany/50 block mb-1">Tab Name *</label>
                    <input value={form.label} onChange={e => f('label', e.target.value)}
                      className="input-field" placeholder="e.g. Gift Sets" required />
                  </div>
                  <div>
                    ...
                    <input value={form.icon} onChange={e => f('icon', e.target.value)}
                      className="input-field text-center text-xl" placeholder="🎁" maxLength={2} />
                  </div>
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  <button type="submit" disabled={saving} className="btn-gold flex-1 flex items-center justify-center gap-2">
                    {saving ? <><div className="w-4 h-4 border-2 border-cream border-t-transparent rounded-full animate-spin" />Saving...</> : editId ? 'Save Changes' : 'Create Tab'}
                  </button>
        ...
        {deleteId && (
          <div className="fixed inset-0 bg-mahogany/60 z-50 flex items-center justify-center px-4">
            <div className="bg-cream p-8 max-w-sm w-full text-center space-y-4">
              <h3 className="font-cinzel font-bold text-xl text-mahogany">Delete Tab?</h3>
              <p className="font-lora text-sm italic text-mahogany/60">
                Products assigned to this tab will not be deleted. They will just not appear under this tab.
              </p>
        ...
        {loading ? (
          ...
          <div className="bg-white border border-parchment-dark p-16 text-center">
            ...
            <h3 className="font-cinzel text-lg text-mahogany mb-2">No tabs yet</h3>
            <button onClick={openNew} className="btn-gold mt-4">Create First Tab</button>
          </div>
        ) : (
          <div className="space-y-3">
            {types.map((t, idx) => (
              <div key={t.id} className={`bg-white border overflow-hidden ${t.is_active ? 'border-parchment-dark' : 'border-parchment-dark opacity-50'}`}>
                <div className="flex items-center gap-4 p-4">
                  {/* Reorder */}
                  <div className="flex flex-col gap-1 shrink-0">
                    <button onClick={() => moveUp(t, idx)} disabled={idx === 0}
                      className="text-mahogany/30 hover:text-gold disabled:opacity-20 transition-colors">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 15l7-7 7 7" />
                      </svg>
                    </button>
                    <button onClick={() => moveDown(t, idx)} disabled={idx === types.length - 1}
                      className="text-mahogany/30 hover:text-gold disabled:opacity-20 transition-colors">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button onClick={() => toggleActive(t)}
                      className={`font-raleway text-xs font-semibold uppercase tracking-wider px-3 py-1.5 border transition-colors ${
                        t.is_active ? 'bg-teal/10 text-teal-dark border-teal/20' : 'bg-mahogany/10 text-mahogany/40 border-mahogany/20'
                      }`}>
